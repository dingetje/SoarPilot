...
#if defined( m68k ) && !defined(memcpy)
...
#define v_cycle(i,j) vf(7,i) += (j ? hf(i) : p[i]) + k_0[i+j] + s_1(vf(4,i)) + ch(vf(4,i),vf(5,i),vf(6,i)); vf(3,i) += vf(7,i); vf(7,i) += s_0(vf(0,i))+ maj(vf(0,i),vf(1,i),vf(2,i))
...
#define one_cycle(a,b,c,d,e,f,g,h,k,w) q(h) += s_1(q(e)) + ch(q(e), q(f), q(g)) + k + w; q(d) += q(h); q(h) += s_0(q(a)) + maj(q(a), q(b), q(c))
...
{   
	...
	sha2_32t padconst1=0xffffff80;
	sha2_32t padconst2=0x00000080;
...
//	DumpCTX(ctx);
    ...
    ctx->wbuf[i >> 2] &= padconst1 << 8 * (~i & 3);
    ctx->wbuf[i >> 2] |= padconst2 << 8 * (~i & 3);
//	DumpCTX(ctx);
