...
*
*********************************************************************/
//Err GPSClose( const UInt16 refNum ) SYS_TRAP(gpsLibTrapClose);
Err GPSClose( const UInt16 refNum ) SYS_TRAP(sysLibTrapCustom);
...
*
*       GPSLibOpen *must* be called before calling any other GPSLib
*       functions, with the exception of GPSGetLibAPIVersion.
*       If an error occurs during initialization, GPSLibOpen returns the
